## Histogrammes de volumes de PSC et d'air froid journaliers 
...
def histogram_volume(nom_fichier,latsize,lonsize):
    data=xr.open_dataset(nom_fichier,engine='pynio')                  # Parfois besoin d'enlever le engine='pynio'
    lat=data.Latitude
    long=data.Longitude
    T=data.Temperature
...
# T<TSTS
    ...
    xlabel('Longitude')
    ylabel('Latitude')
    plt.title(u'Volume de T<TSTS')
  
# PSC NAT   
    mapVnat=np.zeros([len(latbin)-1,len(longbin)-1])
...
# T<TNAT
    ...
    xlabel('Longitude')
    ylabel('Latitude')
    plt.title(u'Volume de T<TNAT')
  
# PSC ICE
    mapVice=np.zeros([len(latbin)-1,len(longbin)-1])
